...
def ploting(dict, mode="minimax", mode_log="on", save=True, show=False, particular_name="std", logyaxis=False):
    ...
    for run in dict.keys():
        ...
        if logyaxis==True:
            ax2.plot(times, np.log10(exp.results[2]),linewidth=3 ,alpha=0.5, label=dict[run]["label"], color=run_color)
        else:
        ...
        if number_bobs>1:
            if mode == "minimax":
                ...
                if logyaxis==False:
                    ...
                    axins.plot(np.log10(exp.results[0][loc1:loc2]), exp.results[2][loc1:loc2], '-', linewidth=3,alpha=.8, color=colors[run], label=dict[run]["label"])
                    axins.fill_between(np.log10(exp.results[0][loc1:loc2]), (exp.results[2][loc1:loc2] - exp.stds[1][loc1:loc2]/2),(exp.results[2][loc1:loc2] + exp.stds[1][loc1:loc2]/2),alpha=.1, facecolor=colors[run])

                    axins.plot(np.log10([exp.results[0][loc1], exp.results[0][loc2-1]]), [exp.optimal_value]*2, '--', alpha=.8, linewidth=5,color=color_2l,
                    label="Optimal 2L")
                    ...
                    # axinticks.append(np.log10(exp.results[2][-1]))
...
ploting(dict_plot,mode_log="on",save=True,logyaxis=False,show=False, particular_name="QL",mode="stds")
