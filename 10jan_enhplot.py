...
def ploting(dict, mode="minimax", mode_log="on", save=True, show=False, particular_name="std", logyaxis=False):
    ...
    for run in dict.keys():
        ...
        if once==True:
            ...
            axins = zoomed_inset_axes(ax2, zoom=4.2,loc="lower right")
        ...
        if number_bobs>1:
            if mode == "minimax":
                ...
                if logyaxis==False:
                    ...
                    axins.plot(np.log10(exp.results[0][loc1:loc2]), exp.results[2][loc1:loc2], '-', linewidth=6,alpha=.6, color=colors[run], label=dict[run]["label"])
                    ...
                    label="Optimal 2L")
                    if (dict[run]["label"] == "UCB-1")|(run=="run_9"):
                    ...
                    axinticks.append(np.log10(exp.results[2][loc2]))
    ...
    ax1.legend()
    mark_inset(ax2, axins, loc1=1, loc2=2, fc="green", ec="0.6", alpha=0.5)
    if logyaxis == True:
...
colors = {"run_1": color1, "run_2": color2, "run_3":color3, "run_4":"purple", "run_9":"red", "run_5":colorucb1, "run_6": colorucb2, "run_7":colorucb3, "run_8":"green"}
