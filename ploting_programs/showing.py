...
def ploting(dict, mode="minimax", mode_log="on", save=True, show=False, particular_name="std"):
    ...
    for run in dict.keys():
        number_phases, amplitude, layers, resolution, searching_method, guessing_rule, method_guess, number_bobs, bound_displacements,efficient_time = dict[run]["info"]
        ...
        if efficient_time == "True":
            ...
            ax3.plot(times, exp.results[3],  '.', color=run_color)
        ...
        ax3.plot(times, exp.results[3],linewidth=4,  alpha=0.8 ,label=dict[run]["label"], color=run_color)

        if once==True:
            ax1.plot(times,exp.optimal_value*np.ones(len(exp.results[0])))
            ax2.plot(times,exp.optimal_value*np.ones(len(times)))
            ax3.plot(times, exp.LRBound(exp.results[0]), '--', linewidth=2, color="black", label="Lai Robins bound")
            # ax3.plot(times, 2+ (8*np.log(exp.results[0])/0.301786670136757), label="UCB UPPER BOUND")
            once=False
        # print(exp.results[3]-neg)
        ...
        if number_bobs>1:
            if mode == "minimax":
                ...
                ax3.plot(times, exp.minimax[4], '--',linewidth=2,  color=run_color)
                ax3.plot(times, exp.minimax[5], '--',linewidth=2,  color=run_color)
                # if self.
                ...
                ax3.fill_between(times, exp.minimax[4], exp.minimax[5], alpha=.2, color=run_color)
                ...
                ax3.plot(times, exp.results[3] - exp.stds[2]/2, '--',linewidth=2,  color=run_color)
                ax3.plot(times, exp.results[3] + exp.stds[2]/2, '--',linewidth=2,  color=run_color)
                ...
                ax3.fill_between(times,  exp.results[3] - exp.stds[2]/2,  exp.results[3] + exp.stds[2]/2, alpha=.2, color=run_color)
            else:
