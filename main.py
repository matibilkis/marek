...
class MegaFrontEnd():
    ...
    def RunInteresting(self, total_episodes=10**3, bob=12):
        dict={}
        # method = "ep-greedy"
        #
        # for tau in [100]:
        ...
        #         dict["run_"+str(c)]["info"] = [exper.number_phases, exper.amplitude, exper.layers, exper.resolution, exper.searching_method, exper.guessing_rule, exper.method_guess, exper.number_bobs, exper.bound_displacements, exper.efficient_time]
        #         dict["run_"+str(c)]["info_ep"] = [exper.ep_method, exper.ep, exper.min_ep, exper.time_tau]
        #         dict["run_"+str(c)]["info_ucb"] = [exper.ucb_method]
        #
        ...
        for soft in [1]:
            exper = training.Experiment(searching_method = method, layers=self.layers ,resolution=self.resolution, bound_displacements=self.bound_displacements, states_wasted=total_episodes, guessing_rule=self.guessing_rule, soft_ts=soft, efficient_time=self.efficient_time)
            exper.train(bob)
    ...
    def RunAll(self, total_episodes=10**3, bob=1):
        ...
        for soft in [0.01, 0.1,1]:
            for mode_ts in ["update_to_q", "None"]:
                ...
                exper = training.Experiment(searching_method = method, layers=self.layers, ep=ep,resolution=self.resolution, bound_displacements=self.bound_displacements, states_wasted=total_episodes, guessing_rule=self.guessing_rule, soft_ts=soft, efficient_time=self.efficient_time, ts_method=mode_ts)
                exper.train(bob)
