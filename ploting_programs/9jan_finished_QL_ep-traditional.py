...
def ploting(dict, mode="minimax", mode_log="on", save=True, show=False, particular_name="std", logyaxis=False):
    ...
    for run in dict.keys():
        ...
        if once==True:
            ...
            if logyaxis == True:
                ax2.plot(times,np.log10(exp.optimal_value)*np.ones(len(exp.results[0])), '--',linewidth=9, alpha=0.8, label=r'$P_*^{(2)}$', color=color_2l)

                ax2.plot([times[0], times[-1]], np.log10([exp.homodyne_limit]*2), '--', linewidth=9 , color="black", label="Homodyne limit")
            else:
                ax2.plot(times,exp.optimal_value*np.ones(len(exp.results[0])), '--',linewidth=9, alpha=0.8, label=r'$P_*^{(2)}$', color=color_2l)
            ...
            loc1=-int(len(exp.results[0])*0.3)
            loc2=-1
            once=False
        ...
        if logyaxis==True:
            ax2.plot(times, np.log10(exp.results[2]),linewidth=4 ,alpha=0.9, label=dict[run]["label"], color=run_color)
        else:
            ax2.plot(times, exp.results[2],linewidth=4 ,alpha=0.9, label=dict[run]["label"], color=run_color)
        ...
        if number_bobs>1:
            if mode == "minimax":
                ...
                if logyaxis==False:
                    ...
                    ax2.fill_between(times,  exp.results[2] - exp.stds[1]/2 ,  exp.results[2] + exp.stds[1]/2, alpha=0.1, color=run_color)
                    ...
                    ax1.fill_between(times, (exp.results[1] - exp.stds[0]/2)/exp.results[0],
                    (exp.results[1] + exp.stds[0]/2)/exp.results[0], alpha=.4, color=run_color)
                    ax2.fill_between(times,  np.log10(exp.results[2] - exp.stds[1]/2) , np.log10(exp.results[2] + exp.stds[1]/2), alpha=0.1, color=run_color)
    ...
    if save == True:
        ...
        if particular_name != "std":
            plt.savefig(str(layers) + "L" + str(phases) + "PH"+str(resolution) + "R/figures/"+particular_name+str(mode)+".pdf")
        else:
...
colors = {"run_1": color1, "run_2": color2, "run_3":color3, "run_6":"purple", "run_8":"orange"}
...
labels = {"run_1":r'$\epsilon = 0.01$'+"-greedy" , "run_2": r'$\epsilon = 0.3$'+"-greedy", "run_3": r'$\epsilon = 1$'+"-greedy", "run_8":"Max(0.01, "+r'$e^{-t/\tau}$'+")-greedy"} #r'$\epsilon = Max(0.01, e^{-t/\tau}$)', "run_3": "1-greedy"}
...
# ,
interesting = ["run_1", "run_2", "run_3","run_8"]
...
ploting(dict_plot,mode_log="on",save=True,logyaxis=False,show=True, particular_name="ep-greedy-QL",mode="stds")
